...
# Normalización mínima por si hay columnas numéricas como texto
numeric_cols = [
...
def build_setup_summary(dff):
    ...
    return html.Div([
        ...
        html.P(
            f"Condiciones medias de la sesión: "
            f"Track {fmt_num(track_temp, 1, ' °C')} | "
            f"Aire {fmt_num(air_temp, 1, ' °C')} | "
            f"Humedad {fmt_num(humidity, 1, '%')}"
        ),
    ])
...
app = Dash(__name__)
app.title = "Dashboard Moto3 Goiânia"
...
app.layout = html.Div(
    [
        html.H1("Dashboard Moto3 — Goiânia 2026"),
        html.P("App conectada a CSV: telemetría, tareas por rol y parámetros de setup."),
            ...
            [
                ...
                html.Div(
                    [
                        ...
                        dcc.Dropdown(
                            id="sesion-dropdown",
                            options=[{"label": s, "value": s} for s in sesiones_disponibles],
        ...
        html.Div(id="setup-summary", style={
            "backgroundColor": "white",
            "padding": "18px",
            "borderRadius": "14px",
            "boxShadow": "0 2px 10px rgba(0,0,0,0.06)"
        ...
        html.Div(id="role-insight", style={
            "backgroundColor": "white",
            "padding": "18px",
            "borderRadius": "14px",
            "boxShadow": "0 2px 10px rgba(0,0,0,0.06)"
...
def update_dashboard(rol, sesion):
    ...
    p_front = dff["presion_front_hot_target_bar"].mean() if "presion_front_hot_target_bar" in dff.columns else None
    p_rear = dff["presion_rear_hot_target_bar"].mean() if "presion_rear_hot_target_bar" in dff.columns else None

    rear_pressure_color = status_color(p_rear, low_alert=1.65)
    anti_squat_color = status_color(anti_squat, good_min=108, good_max=112)
    thermal_color = status_color(temp_right, high_alert=95)
    ...
    cards = [
        ...
        card("Temp. flanco derecho", fmt_num(temp_right, 1, " °C"), "Control térmico neumático", color=thermal_color),
        card("Anti-squat", fmt_num(anti_squat, 0, " %"), "Objetivo 108–112%", color=anti_squat_color),
        card("Presión trasera hot", fmt_num(p_rear, 2, " bar"), "Mínimo legal Race: 1.65", color=rear_pressure_color),
    ]
    ...
    ) if not dff.empty else go.Figure()
    ...
    sectors_df = dff.melt(
        ...
        var_name="sector",
        value_name="tiempo"
    ) if not dff.empty else pd.DataFrame(columns=["vuelta", "run", "sector", "tiempo"])
    ...
    ) if not sectors_df.empty else go.Figure()
    ...
    # Mapas electrónicos
    # ----------------------------
    maps_df = dff[["vuelta", "traction_control_lvl", "engine_brake_lvl", "anti_wheelie_lvl"]] if not dff.empty else pd.DataFrame()
    fig_maps = go.Figure()
    if not maps_df.empty:
        fig_maps.add_trace(go.Scatter(x=maps_df["vuelta"], y=maps_df["traction_control_lvl"], mode="lines+markers", name="TC"))
        fig_maps.add_trace(go.Scatter(x=maps_df["vuelta"], y=maps_df["engine_brake_lvl"], mode="lines+markers", name="EBC"))
        fig_maps.add_trace(go.Scatter(x=maps_df["vuelta"], y=maps_df["anti_wheelie_lvl"], mode="lines+markers", name="AWC"))
    ...
    insights = {
        "Piloto": (
            "Concéntrate en la estabilidad en T1, conservar el flanco derecho del trasero y modular gas "
            "en las primeras vueltas para llegar con tracción al final."
        ),
        "Ingeniero de Pista": (
            "Cruza sectores, velocidad punta, presiones y anti-squat para decidir si el setup mantiene el "
            "equilibrio entre estabilidad en recta y agilidad en el mixto."
        ),
        "Telemétrico": (
            "Prioriza el cruce entre temperatura de pista, presiones dinámicas, temperatura L/C/R del neumático "
            "y mapas electrónicos para validar la correlación con el modelo base."
        ),
        "Jefe de Mecánicos": (
            "Tu foco es operativo: swap de neumáticos, chequeo de torque, estado de suspensiones y protocolo "
            "sin errores entre runs."
        ),
        "Técnico de Neumáticos": (
            "Vigila especialmente el SC1 trasero y el flanco derecho. La clave es mantener presión legal y "
            "evitar sobretemperatura o degradación prematura."
        )
    }
    ...
    return (
        cards, fig_lap, fig_sector, fig_thermal, fig_pressure,
        fig_maps, fig_compound, kanban, summary, role_insight
    )
